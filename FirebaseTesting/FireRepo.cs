...
namespace FirebaseTesting
{
    public class FireRepo<T> : IFireRepo<T>
    {
        ...
        {
            FirebaseResponse resp = await this.client.GetTaskAsync(path);
        ...
        public async Task Add(T Model,Guid id)
        {
            ...
            SetResponse setInsert = await this.client.SetTaskAsync(savePath,Model);
        }

        public async Task Delete(Guid id)
        {
            FirebaseResponse deleteResp
                = await this.client.DeleteTaskAsync($"{this.path}/{id}");
        }
        ...
        public async Task<T> Update(Guid id,T Model)
        {
            ...
            FirebaseResponse response
                = await client.UpdateTaskAsync(findPath, Model);
